...
function saveUploadFile({ fieldName, fileName, fileBuffer }) {
  ...
  const relativePath = path.posix.join("/uploads/setup", safeName);
  const diskPath = path.join(process.cwd(), "public", "uploads", "setup", safeName);
...
function setupUpload(req, res, next) {
  ...
  if (!boundaryMatch) {
    ...
    return res.redirect(req.get("referer") || "/setup/classes");
  }
  ...
  req.on("data", (chunk) => {
    ...
    if (total > MAX_UPLOAD_BYTES) {
      req.destroy(new Error("Fichier trop volumineux (max 20 MB)"));
      return;
    }
    chunks.push(chunk);
  });
    ...
    try {
      ...
      parts.forEach((part) => {
        ...
        if (part.fileName) {
          ...
          const savedPath = saveUploadFile({
      ...
      req.flash("error", err.message || "Erreur traitement fichier");
      return res.redirect(req.get("referer") || "/setup/classes");
    }
  });
}

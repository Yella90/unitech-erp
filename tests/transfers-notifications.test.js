...
test.before(async () => {
  tempDir = await fs.mkdtemp(path.join(os.tmpdir(), "erp-tests-"));
...
test("transfert accepte: frais auto + notifications + isolation detail", async () => {
  ...
  const incomingSource = await TransfersService.listIncomingTransfers({ schoolId: 1, status: "pending" });
  assert.equal(incomingSource.length, 0);
  ...
  await assert.rejects(
    ...
    /Transfert introuvable|Acces|autorise/i
  );
...
test("transfert refuse + notifications correspondantes", async () => {
  const created = await TransfersService.requestTransfer({
    sourceSchoolId: 1,
    toSchoolId: 2,
    matricule: "E002",
...
test("retards mensuels: generation notifications + non duplication + multi-tenant", async () => {

...
let centralPublicVisitsEnsured = false;
const syncStatus = {
...
async function ensureCentralPublicVisitsTable() {
  if (centralPublicVisitsEnsured) return;
  const p = getPool();
  await p.query(`
  ...
  `);
  await p.query("ALTER TABLE public_visits ADD COLUMN IF NOT EXISTS uuid TEXT");
  await p.query("ALTER TABLE public_visits ADD COLUMN IF NOT EXISTS updated_at TIMESTAMP");
  await p.query("ALTER TABLE public_visits ADD COLUMN IF NOT EXISTS deleted_at TIMESTAMP");
  await p.query("ALTER TABLE public_visits ADD COLUMN IF NOT EXISTS version INTEGER NOT NULL DEFAULT 1");
  await p.query("ALTER TABLE public_visits ADD COLUMN IF NOT EXISTS school_id BIGINT NOT NULL DEFAULT 0");
  await p.query("ALTER TABLE public_visits ADD COLUMN IF NOT EXISTS synced_to_central INTEGER NOT NULL DEFAULT 0");
  await p.query("ALTER TABLE public_visits ADD COLUMN IF NOT EXISTS sync_synced_at TIMESTAMP");
  await p.query("CREATE INDEX IF NOT EXISTS idx_public_visits_created ON public_visits (created_at DESC)");
  await p.query("CREATE INDEX IF NOT EXISTS idx_public_visits_page ON public_visits (page_path, created_at DESC)");
  await p.query("CREATE INDEX IF NOT EXISTS idx_public_visits_token ON public_visits (visitor_token)");
  await p.query("CREATE UNIQUE INDEX IF NOT EXISTS idx_public_visits_uuid_unique ON public_visits (uuid) WHERE COALESCE(trim(uuid), '') <> ''");
  centralPublicVisitsEnsured = true;
  centralColumnsCache.delete("public_visits");
}
...
async function processPublicVisitsDirect() {
  await ensureCentralPublicVisitsTable();
  const localColumns = await getLocalColumns("public_visits");
  if (!localColumns.length) return;
  if (!localColumns.includes("uuid")) return;
  if (!localColumns.includes("synced_to_central")) return;
  const centralColumns = await getCentralColumns("public_visits");
  if (!centralColumns.has("uuid")) return;

  const pending = await all(
    `
      SELECT *
      FROM public_visits
      WHERE COALESCE(synced_to_central, 0) = 0
      ORDER BY id ASC
      LIMIT 200
    `
  );
  ...
  for (const row of pending || []) {
    try {
      ...
      await upsertCentralFromLocal("public_visits", row, "update");
      // eslint-disable-next-line no-await-in-loop
      await run(
        `
          UPDATE public_visits
          SET synced_to_central = 1,
              sync_synced_at = CURRENT_TIMESTAMP,
...
async function syncTick() {
  ...
  try {
    await pingCentral();
    await processPublicVisitsDirect();
